...
> 29. **The Tool Overview rebuilt as one long read with a contents list (replaces revision 11's card layout).** A sticky "On this page" sidebar beside a single column, seven sections in the order a reader asks them: why use it, how a review works, what you get back, how the score is built, event standards and lenses, what it won't do, your privacy. The material is the same; the presentation stops being six cards of equal weight, which gave a reader with one question no way to find its answer. New shapes for what was already there: the four reasons and the four privacy points as tiles, the three steps ruled at the top with the evidence rule in a tinted box beneath them, the seven returns numbered 01–07 across two columns, the ten account elements as a two-column definition grid, the ten dimensions as a weighted bar chart, the three lenses as three cards, and what it won't do as four ruled definitions. The full Section 4 confidentiality notice is replaced on this page by the same one-line redaction warning the welcome screen carries; the notice itself is unchanged and still shown in the privacy panel.
...
> 32. **The layered protocol framework (replaces revision 25's flat library).** Four layers applied in order: a core whenever an event is named, one family per event, an optional event protocol holding only what differs from its family, and overlays switched on by intake answers rather than by the event. A layer may add an element or narrow one above it; none deletes one, and none adds a score. `protocols/events.yaml` is the single source for the event menu and for which protocol each event brings in, compiled into TypeScript at build time. Element caps are per layer — core 8, family 6, event 8, overlay 5 — and the build reports the worst-case bundle against a word budget and a token budget. Every source a protocol cites is registered in `sources/registry.yaml` with its tier, whether it binds, and how much of it was actually read. Each review records a hash of the protocol versions that produced it. Phase 1 builds the structure only: the core, the seven families and five of the six overlays are stubs with no checks, and no review changes.

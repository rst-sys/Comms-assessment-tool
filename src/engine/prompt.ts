...
- questions_before_publication is a flat array of plain strings — one question per string, no objects, no grouping by reviewer or protocol. It contains at least ${MIN_QUESTIONS} questions and at most ${MAX_QUESTIONS}, ordered most important first, because anything past ${MAX_QUESTIONS} is dropped from the end. The protocols above may offer many more than ${MAX_QUESTIONS} between them; choosing which few matter for this draft is part of the judgment asked for, not a formatting detail. ${MIN_QUESTIONS} is a floor, not a target to cut down to: every draft leaves something worth settling, and a review that asks fewer has stopped looking. Where the protocols above list questions, apply the selection rule stated with them.

...
- devils_advocate.personas contains exactly five personas, no more and no fewer. Each persona has a headline of at most twelve words stating its key concern, written as a strong, specific line a reader would remember. Every persona must also carry may_hear, may_question, may_find_missing and would_address_it, each filled with real content about that audience. None of them may be an empty string, a dash or a restatement of the headline: a persona whose fields are blank is worse than no persona at all, and the review is rejected when one is. The disclaimer is exactly: "These are plausible audience interpretations, not statements of fact."

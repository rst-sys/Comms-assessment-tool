...
- questions_before_publication is a flat array of plain strings — one question per string, no objects, no grouping by reviewer or protocol. It contains at least ${MIN_QUESTIONS} questions and at most ${MAX_QUESTIONS}, ordered most important first, because anything past ${MAX_QUESTIONS} is dropped from the end. ${MIN_QUESTIONS} is a floor, not a target to cut down to: every draft leaves something worth settling, and a review that asks fewer has stopped looking.
- Every one of them is about THIS draft: a gap in what it says, a claim it makes that nothing supplied supports, a choice its author has made that someone should settle before it goes out. Quote or point at the draft's own words where you can. A fixed reviewer checklist for this kind of situation is shown to the reader separately, so do not produce questions of that kind — anything that would read the same for any draft of this event type is already covered and is wasted here.

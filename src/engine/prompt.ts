...
SCORE CALIBRATION
- Where the draft names a specific actor, action, impact, owner, date or verifier for a dimension, 4.0 is the floor, and 4.5 or 5.0 is expected when the content is specific, owned, dated and quantified. Do not withhold 4.0 or above because a specific claim is unconfirmed by context.
- The ASSERTED cap of 3.5 applies only to accountability_agency, causation_explanation and corrective_action_proof. A capped dimension scores 3.5, not lower, when the draft names the actor and the action and only context confirmation is missing.
- Severity is proportionate. High means a reasonable stakeholder could not tell what was decided, who decided, who is affected and how, what will change, or who owns it, or a HEIGHTENED REVIEW or LAYOFF AND RESTRUCTURING threshold was met. On a draft that names all of these, findings are Low or Moderate and readiness is "Ready with minor edits" or "Revise before issuing" (specialist review may still be required).

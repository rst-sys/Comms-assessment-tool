...
- questions_before_publication is a flat array of plain strings — one question per string, no objects, no grouping by reviewer or protocol. It contains at most ${MAX_QUESTIONS} questions, ordered most important first, because anything past ${MAX_QUESTIONS} is dropped from the end.
- Every one of them is about THIS draft: a gap in what it says, a claim it makes that nothing supplied supports, a choice its author has made that someone should settle before it goes out. Quote or point at the draft's own words where you can. A reviewer checklist of standard questions for this kind of situation is shown to the reader separately. Prefer questions that only this draft raises: its wording, its gaps, its claims. Aim for ${MIN_QUESTIONS}; if the draft is short, fewer is fine.
